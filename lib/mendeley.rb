...
class Mendeley
  ...
  def initialize(consumer_key=$default_key, consumer_secret=$default_secret, site=$default_site)

    if File.exist?("auth.yaml")
      auth_contents = YAML::load(File.open("auth.yaml"))
      $access_token = auth_contents[:access_token]
    end
    $access_token = Authorise.oapi_authorise(consumer_key, consumer_secret, site) if $access_token == nil
    puts $access_token
  end
